...
from os import F_OK, O_CREAT, O_EXCL, O_NOFOLLOW, O_RDONLY, O_WRONLY, getpid, unlink
...
from fruitbak.pool.agent import PoolAction, PoolReadahead
from fruitbak.pool.storage import Storage
from fruitbak.util import Initializer, fallback, fd, initializer, sysopen, sysopendir
...
class Filesystem(Storage):
    ...
    def is_valid_file_name(self):
        return regcomp(
            '[A-Za-z0-9'
            + b64escaped
            + ']{%d}' % (len(my_b64encode(bytes(self.hash_size))) - 2,)
        ).fullmatch
    ...
    class NamedTemporaryFile:
        def __init__(self, path, mode=0o666, *, dir_fd=None):
            ...
            self.dir_fd = dir_fd
            self.fd = sysopen(
                path, O_WRONLY | O_EXCL | O_CREAT | O_NOFOLLOW, 0o440, dir_fd=dir_fd
            )
    ...
    def lister(self, agent):
        ...
        with FilesystemListahead(
            filesystem=self, agent=agent, iterator=iter(dirs)
        ) as listahead:
                ...
                try:
                    if not pooldir_fd.access(path, F_OK):
                        ...
                        with tmp as fd:
                            ...
                            fd.sync()
                            proc_self_fd.link(
                                str(fd), path, dir_fd=pooldir_fd, exist_ok=True
                            )
                except:
                    callback(exc_info())
                else:
