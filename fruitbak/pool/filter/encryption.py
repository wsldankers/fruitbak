...
class Encrypt(Filter):
    ...
    def validated_key(self):
        ...
        if key is None:
            ...
            raise RuntimeError(
                "No encryption key configured. Add this to your configuration:\npool_encryption_key = %r"
        ...
        if isinstance(key, str):
            ...
            try:
                ...
                raise TypeError(
                    "encryption key must be bytes-like, not %r" % (type(key).__name__,)
                ) from None
            key = bytes(key)
        if len(key) != 32:
            raise TypeError(
                "encryption key must be 32 bytes long, not %d" % (len(key),)
            )
        return key
    ...
    def aes(self):
        ...
        if hash_size_bits % block_size_bits:
            raise RuntimeError(
                "size of hash function (%d bits) is incompatible with AES block size (%d bits)"
