...
try:
    ...
    class XZ(Compressor):
        ...
        def compress(self):
            ...
            return lambda value: lzma.compress(
                value, format=lzma.FORMAT_RAW, filters=filters
            )

        @initializer
        def decompress(self):
            filters = self.filters
            return lambda value: lzma.decompress(
                value, format=lzma.FORMAT_RAW, filters=filters
            )
...
try:
    ...
    class LZ4Block(Compressor):
        ...
        def compress(self):
            preset = self.preset
            if preset < 0:
                return lambda value: lz4.block.compress(
                    value, mode='fast', acceleration=-preset
                )
            elif preset > 0:
                return lambda value: lz4.block.compress(
                    value, mode='high_compression', compression=preset
                )
            else:
...
try:
    ...
    class LZ4Frame(Compressor):
        ...
        def compress(self):
            ...
            return lambda value: lz4.frame.compress(
                value, block_size=block_size, compression_level=preset, store_size=False
            )
