...
from os import fwalk, rename, rmdir, unlink
from pathlib import Path

from hashset import Hashset

from fruitbak.config import configurable, configurable_command, configurable_function
from fruitbak.new.share import NewShare
from fruitbak.transfer import LocalTransfer
from fruitbak.util import Initializer, initializer, time_ns, xyzzy
...
class NewBackup(Initializer):
    ...
    def sharedir_fd(self):
        ...
        return self.backupdir_fd.sysopendir(
            self.sharedir, create_ok=True, path_only=True
        )
    ...
    def backup(self):
        ...
        with backupdir_fd.flock(LOCK_EX | LOCK_NB):
            ...
            for root, dirs, files, root_fd in fwalk(
                dir_fd=backupdir_fd, topdown=False, onerror=onerror
            ):
                for name in files:
