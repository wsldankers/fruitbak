...
from fcntl import LOCK_EX, LOCK_NB, LOCK_SH, LOCK_UN, flock as fcntl_flock
from os import (
    O_CLOEXEC,
    O_DIRECTORY,
    O_NOCTTY,
...
from fruitbak.util.oo import Initializer, flexiblemethod, initializer
...
class fd(int):
    ...
    def sysopendir(
        cls, path, dir_fd=None, mode=0o777, path_only=False, create_ok=False, **kwargs
    ):
        ...
        if create_ok:
            try:
                return cls.sysopen(
                    path, flags, dir_fd=dir_fd, large_file=False, **kwargs
                )
            except FileNotFoundError:
                try:
    ...
    def opener(self, mode=0o666, **kwargs):
        ...
        def opener(path, flags):
            ...
            return os_open(
                path, flags | O_CLOEXEC | O_NOCTTY, mode=mode, dir_fd=self, **kwargs
            )
    ...
    def rename(self, src, dst, *, dir_fd=None, **kwargs):
        ...
        return os_rename(
            unpath(src), unpath(dst), src_dir_fd=self, dst_dir_fd=dir_fd, **kwargs
        )
