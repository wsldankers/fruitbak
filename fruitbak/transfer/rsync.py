...
from fruitbak.util import Initializer, convert_env, ensure_bytes, initializer, merge_env
...
class RsyncTransfer(Transfer):
    ...
    def transfer(self):
        ...
        def entry_callback(
            ...
            dentry = Dentry(
                name=name, size=size, mtime=mtime, mode=mode, uid=uid, gid=gid
            )
            if hardlink is None:
                if dentry.is_file:
        ...
        def error_callback(line, code=None):
            if not line.startswith(b'file has vanished:') and not line.startswith(
                b'directory has vanished:'
            ):
                print(
                    host_name + ":",
                    str(line, 'UTF-8', 'surrogateescape'),
                    end='',
                    flush=True,
                    file=stderr,
                )
