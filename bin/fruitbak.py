...
from os import environ, fsdecode, getuid, initgroups, mkdir, setresgid, setresuid
from os.path import basename
from pathlib import Path
from pwd import getpwnam
from stat import *
from sys import setswitchinterval, stderr, stdout
from tarfile import (
    BLKTYPE,
    BLOCKSIZE,
    CHRTYPE,
...
def ls(command, host, backup, share, path, max_age):
    ...
    if host is None:
        ...
        print(
            tabulate(
                pmap(backup_info, fbak[host]), headings=headings, alignment={3: True}
            )
        )
        ...
        print(
            tabulate(
                pmap(share_info, fbak[host][backup]),
                headings=headings,
                alignment={4: True},
            )
        )
        ...
        def dentry_info(dentry):
            ...
            mode_chars = ''.join(
                (
                    ...
                    ('s' if mode & S_IXUSR else 'S')
                    if mode & S_ISUID
                    else ('x' if mode & S_IXUSR else '-'),
                    'r' if mode & S_IRGRP else '-',
                    'w' if mode & S_IWGRP else '-',
                    ('s' if mode & S_IXGRP else 'S')
                    if mode & S_ISGID
                    else ('x' if mode & S_IXGRP else '-'),
                    'r' if mode & S_IROTH else '-',
                    'w' if mode & S_IWOTH else '-',
                    ('t' if mode & S_IXOTH else 'T')
                    if mode & S_ISVTX
                    else ('x' if mode & S_IXOTH else '-'),
                )
            )
...
def backup(command, hosts, all, full):
    ...
    full_cutoff = (
        None if isinstance(full, bool) else now - parse_interval(full, False, now)
    )
    auto_cutoff = (
        None if isinstance(all, bool) else now - parse_interval(all, False, now)
    )
...
def gc(command, dry_run, check):
    ...
    with ExitStack() as exit_stack:
        if check:
            ...
            exit_stack.callback(
                rootdir_fd.unlink, missing_filename_tmp, missing_ok=True
            )
...
try:
    from fruitbak.fuse import FUSE, FruitFuse
except ImportError:
    pass
else:

    @cli.command()
    @cli.argument('-o', default='', metavar='options', help="FUSE mount options")
    ...
    def fuse(command, mountpoint, o):
        ...
        FUSE(
            fruit_fuse,
            mountpoint,
            fsname=f'fruitbak:{fbak.rootdir}',
            encoding=fruit_fuse.encoding,
            **options,
        )
...
def nagioscheck(command, max_age):
    ...
    failed_hosts = [name for name, healthy in pmap(info, fbak) if not healthy]
    if failed_hosts:
